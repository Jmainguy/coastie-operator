...
func getPodsReadyTime(r *ReconcileCoastieService, name, namespace string, reqLogger logr.Logger, dsct string) {
    ...
    for _, v := range podList.Items {
        for _, pv := range v.Status.Conditions {
            if pv.Type == "Ready" {
                timeToStart := pv.LastTransitionTime.Sub(t).String()
                reqLogger.Info("Pod Times", "Pod.Name", v.Name, "Pod.TimeToStart", timeToStart, "NodeName", v.Spec.NodeName, "Namespace", namespace, "Name", name)
            }
        }
    }
